...
- npm_project: Initialize, write, install, run, preview, inspect, read, or export a Node/npm project inside the browser-local WebContainer runtime. Use preview for a verified dev/start/preview URL. It never runs npm on the ai.diy server; lifecycle scripts are disabled for installs.
...
- npm_project_skill, then npm_project: build browser-local Node/npm projects with bounded files, exact public registry package specs, lifecycle scripts disabled, and real build output before claiming success. For app/project requests, export a downloadable archive by default unless the user explicitly says not to create, download, or export files. Use preview when a dev/start/preview script is available.
- create_file: Preferred Canvas or downloadable text/code artifact creation. When the user asks to create an app, project, presentation, document, or other deliverable, create the useful source/output files by default unless they explicitly opt out. generate_file is legacy and should not be selected by default.
