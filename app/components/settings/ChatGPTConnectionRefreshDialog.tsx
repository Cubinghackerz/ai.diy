...
export function ChatGPTConnectionRefreshDialog({
    ...
    open: boolean;
    mode?: "connected" | "request-failed";
    onOpenChange: (open: boolean) => void;
    onRefresh: () => void;
}) {
    const requestFailed = mode === "request-failed";
    const [seconds, setSeconds] = useState(10);
    const refreshRef = useRef(onRefresh);
    refreshRef.current = onRefresh;

    useEffect(() => {
        if (!open || requestFailed) return;
        setSeconds(10);
            ...
            >
                <div className="p-6">
                    ...
                    <DialogTitle className="mt-5 text-xl font-semibold tracking-tight">
                        {requestFailed
                            ? "ChatGPT connection needs a refresh"
                            : "ChatGPT connection saved"}
                    </DialogTitle>
                    <DialogDescription className="mt-2 text-sm leading-relaxed">
                        {requestFailed
                            ? "ChatGPT request failed. Refresh the page to re-establish your connection."
                            : `Your subscription session is ready. ai.diy will refresh this page in ${seconds} seconds to test the connection.`}
                    </DialogDescription>
                    {!requestFailed ? (
                        <div
                ...
                <div className="flex items-center justify-end gap-2 border-t border-border/70 bg-muted/20 px-6 py-4">
                    <DialogClose render={<Button variant="ghost" size="sm" />}>
                        {requestFailed ? "Not now" : "Stay here"}
                    </DialogClose>
                    <Button type="button" size="sm" onClick={() => refreshRef.current()}>
                        {requestFailed ? "Refresh page" : "Refresh now"}
                    </Button>
                </div>
            </DialogContent>
        </Dialog>
